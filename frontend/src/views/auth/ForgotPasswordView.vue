...
const handleEmailSubmit = async () => {
  ...
  try {
    ...
    // Always navigate to OTP page, but inform user
    router.push({ name: 'verify-otp', query: { email: email.value } })
  } catch (error: any) {
    console.error('Forgot password error:', error)
    emailError.value = 'Failed to process request. Please try again.'
  } finally {
    loading.value = false
  }
}
          ...
          <form class="auth-form" @submit.prevent="handleEmailSubmit">
            <label class="field">
              ...
              <div class="input-wrap">
                <input v-model="email" type="email" placeholder="curator@heritage.kh" required />
              </div>
            </label>
